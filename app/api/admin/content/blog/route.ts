...
export async function GET() {
  ...
  const items = await getContentList("blog");
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const dup = await checkDuplicateSlug("blog", data.slug);
    if (dup) return Response.json({ error: "A blog post with this slug already exists." }, { status: 409 });
    const now = new Date().toISOString();
    const item: ContentItem = {
      type: "blog", title: data.title, slug: data.slug, excerpt: data.excerpt || "",
