...
export default function AdminEditResourcePage() {
  ...
  useEffect(() => {
    ...
    const fetchResource = async () => {
      try {
        const res = await fetch(`/api/admin/content/resources/${slug}`);
        if (res.status === 401) {
        ...
        const resource = data.item || data.resource || data;
        setForm({
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (data.errors) {
          ...
          setErrors({ _form: data.message || "Failed to update resource" });
        }
        return;
      }
  ...
  const handleArchive = async () => {
    ...
    setArchiving(true);
    try {
      const res = await fetch(`/api/admin/content/resources/${slug}`, { method: "DELETE" });
      if (res.status === 401) {
        router.push("/admin/login");
        return;
      }
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        alert(data.message || "Failed to archive resource");
        return;
      }
      router.push("/admin/content/resources");
    } catch (err: unknown) {
    ...
    return (
      <div className="px-6 py-8">
        <div className="rounded-lg border border-slate-200 bg-white p-12 text-center">
          ...
          <p className="mt-2 text-sm text-slate-500">The resource with slug &quot;{slug}&quot; does not exist.</p>
          <Link href="/admin/content/resources" className="mt-4 inline-block text-sm text-blue-600 hover:text-blue-800">
    ...
    return (
      <div className="px-6 py-8">
        <div className="rounded-lg border border-red-200 bg-red-50 p-8 text-center">
          <h2 className="text-lg font-bold text-red-800">Failed to Load Resource</h2>
          ...
          <div className="mt-4 flex justify-center gap-3">
            ...
            <Link href="/admin/content/resources" className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm text-slate-600 transition hover:bg-slate-50">&larr; Back to Resources</Link>
          </div>
        </div>
      </div>
    );
